...
func callUnaryEcho(c pb.GreeterClient, message string) {
	ctx, cancel := context.WithTimeout(context.Background(), time.Second)
	defer cancel()
	r, err := c.SayHello(ctx, &pb.HelloRequest1{Name: message})
	if err != nil {
...
func main() {
	// target传入是一个字符串，统一格式为scheme://authority/endpoint
	passthroughConn, err := grpc.Dial(
		// passthrough 也是gRPC内置的一个scheme
		fmt.Sprintf("passthrough:///%s", backendAddr), // Dial to "passthrough:///localhost:50051"
		grpc.WithInsecure(),
		grpc.WithTimeout(time.Second*100000),
		// grpc.WithBlock(),
	)
	if err != nil {
